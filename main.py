...
def change_perspective(image):
    h = image.shape[0]
    w = image.shape[1]
    source = [(w/2 - w*0.3,h - h*0.05),(w/2 -w/8.5,h/2+h*0.28),
              (w/2 + w/12,h/2+h*0.28), (w/2 + w/5,h - h *0.05)] ##Cambiar los puntos de referencia
...
def inv_perspective(image):
    h = image.shape[0]
    w = image.shape[1]
    dest = [(w/2 - w*0.3,h - h*0.05),(w/2 -w/8.5,h/2+h*0.28),
              (w/2 + w/12,h/2+h*0.28), (w/2 + w/5,h - h *0.05)] ##Cambiar los puntos de referencia
